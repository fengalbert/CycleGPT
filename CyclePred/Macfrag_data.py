...
from MacFrag_search import MacFrag
from MacFrag_search import searchBonds
...
def Get_MacFrag_Bonds(mol):
    MacFrag_bonds = list()
    MacFrag_bonds_rules = list()
    bonds = list(searchBonds(mol, maxSR=12))  # macfrag
    # bonds_tmp = FindBRICSBonds(mol)
    # bonds = [b for b in bonds_tmp]
    for item in bonds:  # item[0] is bond, item[1] is brics type
        MacFrag_bonds.append([int(item[0][0]), int(item[0][1])])
        MacFrag_bonds_rules.append([[int(item[0][0]), int(item[0][1])], Get_MacFrag_BondFeature([item[1][0], item[1][1]])])
        MacFrag_bonds.append([int(item[0][1]), int(item[0][0])])
        MacFrag_bonds_rules.append([[int(item[0][1]), int(item[0][0])], Get_MacFrag_BondFeature([item[1][1], item[1][0]])])

    result = []
    for bond in mol.GetBonds():
        beginatom = bond.GetBeginAtomIdx()
        endatom = bond.GetEndAtomIdx()
        if [beginatom, endatom] in MacFrag_bonds:
            result.append([bond.GetIdx(), beginatom, endatom])

    return result, MacFrag_bonds_rules

def Get_MacFrag_BondFeature(action):
    result = []
...
def GetFragmentFeats(mol):
    # frags_idx_lst = MacFrag(mol)  # macfrag 
    mol = mol_with_atom_index(mol)
    Macrfrag_bonds = list(searchBonds(mol, maxSR=12))  # 12
    Macrfrag_bonds_index = [mol.GetBondBetweenAtoms(i[0][0], i[0][1]).GetIdx() for i in Macrfrag_bonds]
    if Macrfrag_bonds_index  == []:
        tmp = mol
    else:
        tmp = Chem.FragmentOnBonds(mol, Macrfrag_bonds_index, addDummies=False)
...
def Mol2HeteroGraph(mol):
    ...
    reac_idx, bbr = Get_MacFrag_Bonds(mol)
...
if __name__=='__main__':
    for seed in [2024]:
        random_split('./data/JAK2_all_deal.csv','data/JAK2/',seed=seed)